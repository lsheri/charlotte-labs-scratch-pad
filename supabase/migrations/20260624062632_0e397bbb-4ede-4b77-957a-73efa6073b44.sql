...
- human_turn_indices: every human turn_index you MUST cover
...
Every turn_index in human_turn_indices MUST appear as the turn_index of
at least one node. The output field covered_turn_indices MUST be the
sorted list of every human turn_index you produced a node for, and MUST
equal human_turn_indices. No skipping, no merging across human turns.
AI turns may be collapsed when they are routine continuations, but any
AI turn that was accepted, rejected, verified, or produced a
deliverable also gets its own node. There is no upper bound on node
count — represent the full session.
...
- For PROMPT, JUDGMENT, VERIFICATION, LOOP, BRANCH (human turns): quote
  MUST be the verbatim opening of the human turn's content from
  raw_thread (no paraphrase, no summary, no em dashes). If the turn is
  longer than 180 chars, take the first 180 chars verbatim.
...
{
  "session_summary": "one sentence describing the shape of the session",
  "nodes": [
    {
      "id": "n1",
...
CONSTRAINTS
- covered_turn_indices MUST equal human_turn_indices (same set, sorted).
...
- If raw_thread is empty, return nodes:[], edges:[],
  covered_turn_indices:[], and set null_reason to a short non-punitive
  explanation.
$PROMPT$,
    version = version + 1,
    updated_at = now()
